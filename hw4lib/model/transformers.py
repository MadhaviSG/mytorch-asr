...
class DecoderOnlyTransformer(nn.Module):
    ...
    def forward(self, padded_targets: torch.Tensor, target_lengths: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, dict]:
        ...
        # Pass through all decoder layers, save attention weights
        runnint_att = {}
...
class EncoderDecoderTransformer(nn.Module):
    ...
    def __init__(
        ...
        # To be compatible with CTCLoss, add log_softmax to the output (See nn.LogSoftmax)
    ...
    def encode(self, padded_sources: torch.Tensor, source_lengths: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, dict]:
        ...
        # You can try optionally skipping positional encoding if using an LSTM based speech embedding
        ...
        # Create CTC inputs dictionary
        # CTCLoss expects: (T, N, C) format for log_probs
        ctc_inputs = {
    ...
    def decode(
        ...
        for i in range(self.num_decoder_layers):
            if self.training and self.layer_drop_rate > 0 and random.random() < self.layer_drop_rate:
                continue
            # Pass through decoder layer
            # Match the signature: forward(x, enc_output, dec_key_padding_mask, enc_key_padding_mask, attn_mask)
            x_dec, self_attn, cross_attn = self.dec_layers[i](
                x_dec,              # x
                encoder_output,     # enc_output
                pad_mask_tgt,       # dec_key_padding_mask
                pad_mask_src,       # enc_key_padding_mask
